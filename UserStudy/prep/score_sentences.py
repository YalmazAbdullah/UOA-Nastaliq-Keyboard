...
LM_DIR = "lm/"
DATA_DIR = '../Data_Discount/transformed_sentences'
...
OUTPUT_CSV_PATH = "results/scored_sentences.csv" 
...
if __name__ == "__main__":
    print("Loading data...")
    try:
        dakshina, _ = read_tsv(DAKSHINA_PATH)
        roUrParl, _ = read_tsv(ROURPARL_PATH)
        sentences_to_score, _ = read_tsv(SUBSET_PATH) # These are the sentences we want to score
    ...
    temp_dir = LM_DIR
    try:
        training_data_path = temp_dir+ "urdu_train.txt"
        arpa_model_path = temp_dir+ f"urdu_{N_GRAM_ORDER}gram.arpa"
        binary_model_path = temp_dir+ f"urdu_{N_GRAM_ORDER}gram.binary"
