...
export default function InputBaseline({ id = -1, targetText ="", setCounter }){   
   ...
   const inputRef = useRef(null);
    ...
    const handleInputChange = (e) => {
        ...
        // if input focused and input provided then start timer
        if (document.activeElement === inputRef.current && startTime === null) {
            setStartTime(new_timestamp)
        }
        ...
        if(new_string===targetText){
            try{
                axios.post("http://127.0.0.1:8000/result", {
                    ...
                    log: keyLog,
                    error_log:errorLog,
                    transposition_count : new_trans,
                    ommission_count : new_omm,
                    substitution_count : new_sub,
                    addition_count : new_add,
                    wpm : (new_timestamp-startTime) / targetText.split("").length
                }, {headers: {
