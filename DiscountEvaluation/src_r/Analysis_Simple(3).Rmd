...
```{r}
library(reshape2)
data <- data3_sentence[, c(24, 7:14, 25)]
# Reshape data from wide to long format
long_df <- melt(data, id.vars = c("ID", "Keyboard"), variable.name = "Hand_Finger", 
                value.name = "Press")
print(long_df)
long_df$Keyboard<-as.factor(long_df$Keyboard)
```

------------------------------------------------------------------------
```{r}
crulp <- subset(data, Keyboard == "CRULP")
crulp <- t(crulp[, -c(1, ncol(crulp))])
ime <- subset(data, Keyboard == "IME")
ime <- t(ime[, -c(1, ncol(ime))])
windows <- subset(data, Keyboard == "WINDOWS")
windows <-t(windows [, -c(1, ncol(windows ))])
factor_comp <- rbind(crulp, ime, windows)
```

```{r}

# WRS2::bwtrim(Press ~ Hand_Finger*Keyboard, id =ID, data = long_df)
...
```{r}
bwtrim(8, 3, t(factor_comp))
```
```{r}
# crulp_new <- t(as.data.frame(rowMeans(crulp)))
# ime_new <- t(as.data.frame(rowMeans(ime)))
# windows_new <- t(as.data.frame(rowMeans(windows)))
# compare <- rbind(crulp_new, ime_new, windows_new)
```

```{r}
# performs multiple comparisons associated with Factor A using the (bootstrap-t) method
bwmcp(3, 8, t(factor_comp))
```

```{r}
# performs multiple comparisons associated with Factor A using the (bootstrap-t) method
bwamcp(3, 8, t(factor_comp))
```


```{r}
# uses method BWBMCP to compare the levels of Factor B. If the argument pool=T, the
# function pools the data for you and then calls the function rmmcp

bwbmcp(8, 3,  t(factor_comp))
```


```{r}
between <- sppba(Press ~ Hand_Finger*Keyboard, id =ID, data = long_df, est = "mom", avg = TRUE, nboot = 1000)
within  <- sppbb(Press ~ Hand_Finger*Keyboard, id =ID, data = long_df, est = "mom", avg = TRUE, nboot = 1000)
int <- sppbi(Press ~ Hand_Finger*Keyboard, id =ID, data = long_df, est = "mom", nboot = 1000)
```
...
```{r}
ezANOVA(long_df, Press, between = Keyboard, within = Hand_Finger, id =ID)
fitF$ANOVA
```

```{r}
#long <- long_df[, -1]
t2way(Press ~ Hand_Finger*Keyboard, data = long)
```
```{r}
psthoc <- mcp2a(Press ~ Hand_Finger*Keyboard, id = ID, data = long_df)
